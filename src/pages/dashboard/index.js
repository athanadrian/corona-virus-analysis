...
const Dashboard = ({ history, location }) => {
  ...
  const selectedCountryFullName = selectedCountry
    ? COUNTRIES_LIST.filter((i) => i.code === selectedCountry)
    : "";
    ...
    const fetchStats = async () => {
      if (!selectedCountry && !getParameterByName("country")) {
        ...
        const ipData = await fetch(
          `https://api.ipdata.co/?api-key=${IP_DATA_KEY}`
        ).then((data) => data.json());
      ...
      try {
        ...
        const details = await fetch(resStats.deaths.detail).then((data) =>
          data.json()
        );
            ...
            <Statistic style={{ margin: 0 }}>
              <Statistic.Value>
                {!detail || loading ? (
                  ...
                  formatThousands(detail.totalConfirmed)
                )}
              </Statistic.Value>
              <Statistic.Label>ΚΡΟΥΣΜΑΤΑ</Statistic.Label>
            </Statistic>
            <Statistic style={{ margin: 0 }}>
              <Statistic.Value>
                {!detail || loading ? (
                  ...
                  formatThousands(detail.totalDeaths)
                )}
              </Statistic.Value>
              <Statistic.Label>ΘΑΝΑΤΟΙ</Statistic.Label>
            </Statistic>
            <Statistic style={{ margin: 0 }}>
              <Statistic.Value>
                {!detail || loading ? (
                ...
                {detail.totalRecovered < 1 && (
                  <Popup
                    ...
                    hideOnScroll
                    trigger={
                      <Icon name="info circle" size="tiny" color="red" />
                    }
                    content={`Κάποιες χώρες δεν παρέχουν στοιχεία αριθμού ατόμων, τα οποία ανάρρωσαν. ${selectedCountryFullName[0].name} είναι μια από αυτές`}
                    size="small"
                  />
                )}
              </Statistic.Value>
              <Statistic.Label>ΑΝΑΡΡΩΣΑΝ</Statistic.Label>
            </Statistic>
          </Statistic.Group>
          {selectedCountry && selectedCountry !== "GLOBAL" && (
            <>
              ...
              {showDetail && (
                <Details detail={detail} selectedCountry={selectedCountry} />
              )}
            </>
          )}
        </div>
  ...
  const filterCountries = () => {
    if (!COUNTRIES_LIST) return <p>Φόρτωση χωρών...</p>;
    return (
      <div style={{ display: "flex", alignItems: "center" }}>
        <p style={{ margin: "0 10px 0 0" }}>Επιλογή:</p>
        <select
          defaultValue=""
  ...
  return (
    <DashboardWrapper>
      <h2 className="title">
        {selectedCountry && selectedCountry !== "GLOBAL"
          ? selectedCountryFullName[0].name
          : ""}
      </h2>
      {filterCountries()}
      {buildStats()}
      ...
      {!message && selectedCountry && selectedCountry !== "GLOBAL" && (
        <>
          <div>
            Πως συμπεριφέρεται η εξάπλωση του COVID-19 τις τελευταίες&nbsp;
            <strong>{timeRange} ημέρες</strong>
          </div>
          <LastCases
            country={
              selectedCountry && selectedCountry !== "GLOBAL"
      ...
      {lastUpdate && (
        <small>
          Τελευταία Ενημέρωση:{" "}
          {parseISO(lastUpdate).toLocaleDateString("GR", {
            weekday: "long",
            year: "numeric",
            month: "long",
            day: "numeric",
